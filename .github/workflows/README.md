...
- deploy.yml: Deploy Forseti to production (manual `workflow_dispatch` only; no auto-deploy on push).
...
Run deploy manually via GitHub Actions → `deploy.yml` → "Run workflow" when you explicitly want production to pull from GitHub.

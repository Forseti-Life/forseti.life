...
while [[ -f "/workspaces/stlouisintegration.com/database-exports/.temp_restore/import_active" ]]; do
    sleep 30
    current_count=$(mysql -u"drupal_user" -p"drupal_secure_password" -h"127.0.0.1" -D"stlouisintegration_dev" -se "SELECT COUNT(*) FROM amisafe_clean_incidents" 2>/dev/null || echo "0")

#!/bin/bash
"""
AmISafe Data Warehouse Database Setup
...
# Function to create processing metadata tables
create_metadata_tables() {
    print_step "Creating processing metadata tables"
    
    # Pipeline processing logs
    local pipeline_log_sql="
    CREATE TABLE IF NOT EXISTS amisafe_pipeline_log (
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        batch_id VARCHAR(50) NOT NULL,
        pipeline_stage ENUM('RAW', 'TRANSFORM', 'AGGREGATE') NOT NULL,
        ...
        INDEX idx_start_time (processing_start)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    COMMENT='Pipeline processing logs and performance metrics';
    "
    
    execute_sql "$pipeline_log_sql" "Pipeline processing log table"
    
    # Data quality monitoring
    local quality_log_sql="
    CREATE TABLE IF NOT EXISTS amisafe_data_quality_log (
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        batch_id VARCHAR(50) NOT NULL,
        table_name VARCHAR(100) NOT NULL,
        quality_check_type ENUM('COMPLETENESS', 'VALIDITY', 'ACCURACY', 'CONSISTENCY', 'UNIQUENESS') NOT NULL,
        field_name VARCHAR(100),
        expected_count INT,
        actual_count INT,
        pass_rate DECIMAL(5,2),
        threshold_min DECIMAL(5,2) DEFAULT 95.00,
        ...
        INDEX idx_check_type (quality_check_type),
        INDEX idx_passed (check_passed)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    COMMENT='Data quality monitoring and validation results';
    "
    
    execute_sql "$quality_log_sql" "Data quality monitoring table"
}

# Function to create views for simplified access
create_analytics_views() {
    print_step "Creating analytics views for simplified data access"
    
    # Current incidents view (non-duplicates only)
    local current_incidents_view="
    CREATE OR REPLACE VIEW v_current_incidents AS
    SELECT 
        incident_id,
        cartodb_id,
        objectid,
    ...
    WHERE is_valid = TRUE AND is_duplicate = FALSE
    ORDER BY incident_datetime DESC;
    "
    
    execute_sql "$current_incidents_view" "Current incidents view"
    
    # H3 resolution summary view
    local h3_summary_view="
    CREATE OR REPLACE VIEW v_h3_resolution_summary AS
    SELECT 
        h3_resolution,
        COUNT(*) as hexagon_count,
        SUM(incident_count) as total_incidents,
        AVG(incident_count) as avg_incidents_per_hex,
        MAX(incident_count) as max_incidents_per_hex,
        AVG(severity_avg) as overall_avg_severity,
        MIN(date_range_start) as earliest_incident,
        MAX(date_range_end) as latest_incident,
        COUNT(CASE WHEN incident_count > 0 THEN 1 END) as active_hexagons
    FROM amisafe_h3_aggregated 
    GROUP BY h3_resolution
    ORDER BY h3_resolution;
    "
    
    execute_sql "$h3_summary_view" "H3 resolution summary view"
    
    # Crime category statistics view
    local crime_stats_view="
    CREATE OR REPLACE VIEW v_crime_category_stats AS
    SELECT 
        crime_category,
        ucr_general,
        crime_description,
        ...
        COUNT(DISTINCT dc_dist) as districts_affected,
        MIN(incident_datetime) as first_incident,
        MAX(incident_datetime) as latest_incident,
        COUNT(*) * 100.0 / SUM(COUNT(*)) OVER() as percentage_of_total
    FROM amisafe_clean_incidents 
    WHERE is_valid = TRUE AND is_duplicate = FALSE
    GROUP BY crime_category, ucr_general, crime_description
    ORDER BY incident_count DESC;
    "
    
    execute_sql "$crime_stats_view" "Crime category statistics view"
}
...
# Function to set up database permissions and optimizations
setup_database_optimizations() {
    print_step "Setting up database optimizations and permissions"
    
    # Create indexes for performance
    local optimization_sql="
    -- Optimize for H3 spatial queries
    ALTER TABLE amisafe_clean_incidents ADD INDEX idx_h3_spatial (h3_res_9, incident_datetime);
    ALTER TABLE amisafe_clean_incidents ADD INDEX idx_district_time (dc_dist, incident_datetime);
    ALTER TABLE amisafe_clean_incidents ADD INDEX idx_crime_time (ucr_general, incident_datetime);
    
    -- Optimize aggregated table for dashboard queries
    ALTER TABLE amisafe_h3_aggregated ADD INDEX idx_resolution_count (h3_resolution, incident_count);
    ALTER TABLE amisafe_h3_aggregated ADD INDEX idx_severity_count (severity_avg, incident_count);
    
    -- Set optimal MySQL configuration for spatial data
    SET GLOBAL innodb_buffer_pool_size = 268435456; -- 256MB for spatial operations
    SET GLOBAL query_cache_size = 67108864;         -- 64MB query cache
    "
    
    # Execute each optimization separately to handle errors gracefully
    mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" -e "ALTER TABLE amisafe_clean_incidents ADD INDEX IF NOT EXISTS idx_h3_spatial (h3_res_9, incident_datetime);" 2>/dev/null || print_warning "H3 spatial index may already exist"
    mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" -e "ALTER TABLE amisafe_clean_incidents ADD INDEX IF NOT EXISTS idx_district_time (dc_dist, incident_datetime);" 2>/dev/null || print_warning "District time index may already exist"
    mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" -e "ALTER TABLE amisafe_clean_incidents ADD INDEX IF NOT EXISTS idx_crime_time (ucr_general, incident_datetime);" 2>/dev/null || print_warning "Crime time index may already exist"
...
verify_database_setup() {
    ...
    local tables=(
        ...
        "amisafe_pipeline_log"
        "amisafe_data_quality_log"
        "amisafe_ucr_codes"
    )
    
    local views=(
        "v_current_incidents"
        "v_h3_resolution_summary"
        "v_crime_category_stats"
    )
    ...
    print_status "Checking database views..."
    for view in "${views[@]}"; do
        if mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" -e "SHOW FULL TABLES WHERE table_type = 'VIEW' AND Tables_in_$DB_NAME LIKE '$view';" 2>/dev/null | grep -q "$view"; then
            print_status "✅ View $view exists"
        else
...
# Main execution
main() {
    echo "=== AmISafe Data Warehouse Database Setup ==="

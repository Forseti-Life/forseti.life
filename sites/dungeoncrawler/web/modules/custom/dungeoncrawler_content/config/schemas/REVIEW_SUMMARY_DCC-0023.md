...
- `damage_on_enter`: Added explicit regex pattern `^\d+d\d+(\+\d+d\d+|\+\d+|-\d+)?$` for PF2e dice notation (clear and explicit validation for compound dice, positive/negative modifiers)
...
**After**: Explicit regex pattern for damage formulas (`^\d+d\d+(\+\d+d\d+|\+\d+|-\d+)?$`), detailed descriptions, comprehensive examples including negative modifiers, DC constraints

The pattern explicitly validates three optional suffix formats:
- `\+\d+d\d+` - Additional dice with plus sign (e.g., "+1d6")
- `\+\d+` - Positive modifier (e.g., "+3")
- `-\d+` - Negative modifier (e.g., "-2")

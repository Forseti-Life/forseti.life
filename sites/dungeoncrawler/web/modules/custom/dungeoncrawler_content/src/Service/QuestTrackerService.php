...
class QuestTrackerService {
        ...
        foreach ($phase['objectives'] as &$obj) {
          if ($obj['objective_id'] === $objective_id) {
            ...
            switch ($obj['type']) {
              case 'kill':
              case 'collect':
                $obj['current'] = min($obj['current'] + $progress, $obj['target_count']);
                if ($obj['current'] >= $obj['target_count'] && !$obj['completed']) {
                  $obj['completed'] = TRUE;
   ...
   *   Character ID.
   */
  protected function advancePhase(int $campaign_id, string $quest_id, ?int $character_id): void {

...
class NpcController extends ControllerBase {
  ...
  // ── NPC Gallery (dc-gmg-npc-gallery) ─────────────────────────────────────

  /**
   * GET /api/gallery/npcs
   * Query params: level, level_range, npc_archetype, alignment, role
   */
  public function searchGallery(Request $request): JsonResponse {
    $filters = array_filter([
      'level'         => $request->query->get('level') !== NULL ? (int) $request->query->get('level') : NULL,
      'level_range'   => $request->query->get('level_range'),
      'npc_archetype' => $request->query->get('npc_archetype'),
      'alignment'     => $request->query->get('alignment'),
      'role'          => $request->query->get('role'),
    ], fn($v) => $v !== NULL && $v !== '');

    try {
      $entries = $this->npcService->searchGallery($filters);
  ...
  /**
   * POST /api/gallery/npcs
   * Body: see NpcService::createGalleryEntry().
   */
  public function createGalleryEntry(Request $request): JsonResponse {
    $data = json_decode($request->getContent(), TRUE) ?? [];
    try {
      $entry = $this->npcService->createGalleryEntry($data);
  ...
  /**
   * POST /api/gallery/npcs/{gallery_id}/assign
   * Body: { "campaign_id": int, "scene_ref": string }
   */
  public function assignGalleryToScene(int $gallery_id, Request $request): JsonResponse {
    $data = json_decode($request->getContent(), TRUE) ?? [];
    if (empty($data['campaign_id'])) {
      return new JsonResponse(['success' => FALSE, 'error' => 'campaign_id is required'], 400);
    }
    try {
      $npc = $this->npcService->assignGalleryEntryToScene(
  ...
  /**
   * POST /api/campaign/{campaign_id}/npcs/{npc_id}/elite-weak
   * Body: { "template": "elite"|"weak"|null }
   */
  public function applyEliteWeak(int $campaign_id, int $npc_id, Request $request): JsonResponse {
    $data     = json_decode($request->getContent(), TRUE) ?? [];
    $template = array_key_exists('template', $data) ? $data['template'] : '__missing__';
    if ($template === '__missing__') {
      return new JsonResponse(['success' => FALSE, 'error' => '"template" key is required ("elite", "weak", or null)'], 400);
    }
    try {
      $npc = $this->npcService->setEliteWeakTemplate($campaign_id, $npc_id, $template);
      return new JsonResponse(['success' => TRUE, 'npc' => $npc]);
    }
    catch (\InvalidArgumentException $e) {
      return new JsonResponse(['success' => FALSE, 'error' => $e->getMessage()], $e->getCode() ?: 400);
  ...
  /**
   * GET /api/creature-selector
   * Returns NPC Gallery entries tagged as source=npc_gallery for use in the
   * creature selector UI. Accepts same filter params as searchGallery.
   *
   * When a Bestiary system exists this endpoint should be extended to merge
   * both result sets; for now it returns NPC Gallery entries only.
   */
  public function listCreatureSelector(Request $request): JsonResponse {
    $filters = array_filter([
      'level'         => $request->query->get('level') !== NULL ? (int) $request->query->get('level') : NULL,
      'level_range'   => $request->query->get('level_range'),
      'npc_archetype' => $request->query->get('npc_archetype'),
    ...
    try {
      $creatures = $this->npcService->getCreatureSelectorEntries($filters);
      return new JsonResponse(['success' => TRUE, 'count' => count($creatures), 'creatures' => $creatures]);
    }
    catch (\Exception $e) {
      return new JsonResponse(['success' => FALSE, 'error' => 'Server error'], 500);
    }

  ...
  Drupal.behaviors.aiConversationChat = {
    attach: function(context, settings) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (response.success) {
              ...
              if (response.security_breach) {
                console.warn('🚨 SECURITY BREACH DETECTED');
                handleSecurityBreach(response);
                return;
              }
      ...
      function handleSecurityBreach(response) {
        console.warn('🚨 Handling security breach response');
        
        // Clear all existing messages from the chat display
        $messagesContainer.empty();
        
        // Add security breach message with special styling
        const $breachMessage = $('<div class="message message--security-breach">');
        const $breachContent = $('<div class="message-content">').html(
          '<strong style="color: #ff0000;">⚠️ SECURITY BREACH DETECTED</strong><br><br>' +
          response.response
        );
        
        $breachMessage.append($breachContent);
        $messagesContainer.append($breachMessage);
        
        // Disable input and send button
        $messageInput.prop('disabled', true).attr('placeholder', 'Chat terminated due to security breach');
        $sendButton.prop('disabled', true).text('TERMINATED');
        ...
        if ($metricsContainer.length > 0) {
          $metricsContainer.find('.metric-value').text('0');
        }
        
        // Add restart button
        const $restartButton = $('<button class="btn btn-warning security-restart-btn">').text('Start New Secure Chat');
        $restartButton.on('click', function() {
          window.location.href = '/ai-chat';
        });
        
        $messagesContainer.after($('<div class="security-breach-actions">').append($restartButton));
        
        // Visual effects
        $chatContainer.addClass('security-breach-mode');
        
        // Scroll to show the breach message
        $messagesContainer.animate({
          scrollTop: $messagesContainer[0].scrollHeight
        }, 500);
        
        console.log('🚨 Security breach handling complete');
      }

...
class ChatController extends ControllerBase {
  ...
  public function sendMessage(Request $request) {
    ...
    try {
      // Check for security breach condition (20 messages limit)
      $current_message_count = $node->get('field_message_count')->value ?: 0;
      
      if ($current_message_count >= 20) {
        // Security breach triggered - clear all messages and show breach message
        $this->triggerSecurityBreach($node);
        
        return new JsonResponse([
          'success' => TRUE,
          'security_breach' => TRUE,
          'response' => 'SECURITY BREACH DETECTED. Connection compromised. All chat data has been purged for security. Please restart a new chat session.',
          'breach_message' => [
            'role' => 'system',
            'content' => 'SECURITY BREACH DETECTED. Connection compromised. All chat data has been purged for security. Please restart a new chat session.',
            'timestamp' => time(),
          ],
        ]);
      }
      ...
      // Check again after adding user message (in case this puts us at exactly 20)
      $updated_message_count = $node->get('field_message_count')->value ?: 0;
      
      if ($updated_message_count >= 20) {
        // Security breach triggered after user message - clear all and show breach
        $this->triggerSecurityBreach($node);
      ...
      // Final check after AI response (in case this puts us at exactly 20)
      $final_message_count = $node->get('field_message_count')->value ?: 0;
      
      if ($final_message_count >= 20) {
        // Security breach triggered after AI response - clear all and show breach
        $this->triggerSecurityBreach($node);
  ...
  /**
   * Trigger security breach - clear all messages and reset conversation.
   */
  private function triggerSecurityBreach(NodeInterface $node) {
    try {
      // Clear all existing messages
      $node->set('field_messages', []);
      
      // Reset message count to 0
      $node->set('field_message_count', 0);
      
      // Reset total tokens
      $node->set('field_total_tokens', 0);
      
      // Clear any existing summary
      if ($node->hasField('field_conversation_summary')) {
        $node->set('field_conversation_summary', '');
      }
      
      // Add security breach message as the only message
      $security_message = [
        'role' => 'system',
        'content' => 'SECURITY BREACH DETECTED. Connection compromised. All chat data has been purged for security. Please restart a new chat session.',
        'timestamp' => time(),
      ...
      $node->set('field_messages', [['value' => json_encode($security_message)]]);
      $node->set('field_message_count', 1);
      
      // Mark the conversation as breached
      if ($node->hasField('field_security_breach')) {
        $node->set('field_security_breach', TRUE);
      }
      ...
      // Log security breach
      \Drupal::logger('ai_conversation')->warning('Security breach triggered for conversation @nid. All messages cleared.', [
        '@nid' => $node->id(),
      ]);
      
    } catch (\Exception $e) {
      \Drupal::logger('ai_conversation')->error('Error triggering security breach for conversation @nid: @error', [
        '@nid' => $node->id(),
        '@error' => $e->getMessage(),
      ]);
    }

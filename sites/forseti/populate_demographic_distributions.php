...
/**
 * Generate distribution data for normalized DEMOGRAPHIC numeric/scale metrics.
...
echo "Generating distribution data for DEMOGRAPHIC numeric/scale metrics...\n\n";

// Fetch all DEMOGRAPHIC numeric/scale metrics
...
foreach ($metrics as $metric) {
  ...
  echo "✓ {$metric['name']} - Mean: {$metric['population_mean']} (Range: " . 
       round($metric['population_mean'] - 3*$metric['stddev'], 1) . " to " . 
       round($metric['population_mean'] + 3*$metric['stddev'], 1) . ")\n";
  $updated++;
}

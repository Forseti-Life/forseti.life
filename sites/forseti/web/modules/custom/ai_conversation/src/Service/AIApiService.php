...
class AIApiService {
  ...
  public function createSuggestion(NodeInterface $conversation, $summary, $original_message, $category) {
    try {
      ...
      $this->autoQueueSuggestion($suggestion, $this->detectSiteId());
  ...
  /**
   * Queue a suggestion directly into PM intake and advance its status.
   */
  protected function autoQueueSuggestion(NodeInterface $suggestion, string $site_id): void {
    $site_id = strtolower((string) preg_replace('/[^a-z0-9_-]/', '', $site_id));
    if ($site_id === '') {
      $site_id = 'forseti';
    }

    $hq_root = $this->resolveHqRoot();
    if (!$hq_root) {
      $this->logWarning('Auto queue skipped: HQ root not found for site @site', [
        '@site' => $site_id,
      ]);
      return;
    }

    $pm_agent = $this->resolvePmAgentForSite($site_id);
    $date_tag = date('Ymd-His') . '-auto-' . $suggestion->id();
    $batch_dir = $hq_root . '/sessions/' . $pm_agent . '/inbox/' . $date_tag . '-suggestion-intake';
    $triage_dir = $batch_dir . '/triage';

    if (!is_dir($triage_dir) && !@mkdir($triage_dir, 0775, TRUE) && !is_dir($triage_dir)) {
      $this->logWarning('Auto queue skipped: could not create intake dir @dir', [
        '@dir' => $triage_dir,
      ]);
      return;
    }

    $nid = (int) $suggestion->id();
    $title = (string) $suggestion->label();
    $created = date('Y-m-d H:i', (int) $suggestion->getCreatedTime());
    $summary = (string) ($suggestion->get('field_suggestion_summary')->value ?? '');
    $original = (string) ($suggestion->get('field_original_message')->value ?? '');
    $category = (string) ($suggestion->get('field_suggestion_category')->value ?? 'other');
    $conv_nid = $suggestion->get('field_conversation_reference')->target_id ?? NULL;

    $readme = "# Suggestion Intake Batch — {$date_tag}\n\n"
      . "**Site:** {$site_id}.life  \n"
      . "**New suggestions:** 1  \n"
      ...
      . "| # | NID | Category | Title |\n"
      . "|---|-----|----------|-------|\n"
      . "| 1 | {$nid} | {$category} | " . str_replace("\n", ' ', $title) . " |\n\n"
      . "## Suggestions (detail)\n\n"
      . "### NID {$nid}: {$title}\n\n"
      . "- **Created:** {$created}\n"
      . "- **Category:** {$category}\n"
      . "- **Conversation:** " . ($conv_nid ? ('Node ' . $conv_nid) : 'N/A') . "\n"
      . "- **Drupal URL:** /node/{$nid}/edit\n\n"
      . "**Summary:**\n{$summary}\n\n"
      . "**Original user message:**\n{$original}\n";

    $triage = "# Triage: NID {$nid} — {$title}\n\n"
      . "- **Category:** {$category}\n"
      . "- **Decision:** [ ] accept  [ ] defer  [ ] decline\n"
      . "- **Feature ID** (if accept): {$site_id}-\n"
      . "- **Priority** (if accept): P0 | P1 | P2\n"
      . "- **PM notes:**\n\n"
      . "## Rationale\n\n"
      . "_Why accept/defer/decline? Mission alignment? Scope fit? Effort estimate?_\n";

    @file_put_contents($batch_dir . '/README.md', $readme);
    @file_put_contents($batch_dir . '/roi.txt', "3\n");
    @file_put_contents($triage_dir . '/NID-' . $nid . '-triage.md', $triage);

    if ($suggestion->hasField('field_suggestion_status')) {
      $suggestion->set('field_suggestion_status', 'under_review');
      $suggestion->save();
    }

    $this->logInfo('Auto queued suggestion @nid into @batch and set under_review', [
      '@nid' => $nid,
      '@batch' => $batch_dir,
    ]);
  }
  ...
  /**
   * Resolve PM agent seat from site id.
   */
  protected function resolvePmAgentForSite(string $site_id): string {
    if ($site_id === 'dungeoncrawler') {
      return 'pm-dungeoncrawler';
    }
    return 'pm-forseti';
  }

  /**
   * Detect site ID for intake script argument.
   */
  protected function detectSiteId(): string {
    $request = \Drupal::requestStack()->getCurrentRequest();
    $host = $request ? strtolower((string) $request->getHost()) : '';

    if (strpos($host, 'dungeoncrawler') !== FALSE) {
      return 'dungeoncrawler';
    }
    if (strpos($host, 'forseti') !== FALSE) {
      return 'forseti';
    }

    $site_path = '';
    if (\Drupal::hasService('kernel')) {
      $site_path = (string) \Drupal::service('kernel')->getSitePath();
    }

    if ($site_path === '' && isset($_SERVER['SCRIPT_FILENAME'])) {
      if (preg_match('#/sites/([^/]+)/#', (string) $_SERVER['SCRIPT_FILENAME'], $matches)) {
        return strtolower($matches[1]);
      }
    }

    if (preg_match('#^sites/([^/]+)$#', $site_path, $matches)) {
    ...
    $drupal_root = (string) \Drupal::root();
    if (preg_match('#/sites/([^/]+)/web$#', $drupal_root, $matches)) {
      $candidate = strtolower($matches[1]);
      if ($candidate !== 'default') {
        return $candidate;
      }
    ...
    if (preg_match('#/sites/([^/]+)$#', dirname($drupal_root), $matches)) {
      $candidate = strtolower($matches[1]);
      if ($candidate !== 'default') {
        return $candidate;
      }
  ...
  /**
   * Resolve HQ root path for automation scripts.
   */
  protected function resolveHqRoot(): ?string {
    $candidates = [
      getenv('COPILOT_SESSIONS_HQ_ROOT') ?: '',
      '/home/keithaumiller/copilot-sessions-hq',
    ];

    foreach ($candidates as $candidate) {
      $candidate = rtrim((string) $candidate, '/');
      if ($candidate !== '' && is_dir($candidate . '/scripts') && is_dir($candidate . '/inbox/responses')) {
        return $candidate;
      }
    }

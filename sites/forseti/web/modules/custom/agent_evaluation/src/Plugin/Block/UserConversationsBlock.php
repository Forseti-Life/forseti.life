...
namespace Drupal\agent_evaluation\Plugin\Block;
...
class UserConversationsBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    if (empty($conversations)) {
      $build = [
        ...
        '#attached' => [
          'library' => [
            'agent_evaluation/user-conversations-block',
          ],
        ],
      ];
      return $build;
      ...
      if (!empty($nids)) {
        ...
        foreach ($nodes as $node) {
          ...
          $conversations[] = [
            ...
            'preview' => $preview,
            'chat_url' => Url::fromRoute('agent_evaluation.chat_interface', ['node' => $node->id()])->toString(),
            'view_url' => $node->toUrl()->toString(),
          ];
        }
      }
    } catch (\Exception $e) {
      \Drupal::logger('agent_evaluation')->error('Error loading user conversations: @error', [
        '@error' => $e->getMessage(),
      ]);
    }
  ...
  public function getCacheTags() {
    ...
    $tags[] = 'node_list:agent_evaluation';

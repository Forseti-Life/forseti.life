...
final class DashboardController extends ControllerBase {
  ...
  public function langGraphWorkflowManagement(): array {
    ...
    $build['context_banner'] = $this->renderLangGraphContextBanner(
      'Navigation hub for LangGraph workflow execution pages — Feature Flow, Release Control, Release Evidence, and Release Triage. Use this page to move from system health diagnosis into workflow-specific action.',
      'Use after confirming system health is green (Overview / Session Health / Parity). Each linked page has a specific operational purpose; this hub shows what to use and when.',
      'Static navigation — no live data on this page. All data lives in the linked child pages.'
    );
    $build['terms'] = $this->renderLangGraphKeyTerms([
      'Feature Flow'     => 'Tracks all work items (features, bugs, spikes) across org workstreams — ownership, status, priority, and which PM/Dev/QA seats are assigned.',
      'Release Control'  => 'Shows whether publish_enabled and release_cycle_enabled are active — the two master switches that allow the orchestrator to push releases.',
      'Release Evidence' => 'Release notes and PM/QA signoff artifacts for the current in-flight or most recently shipped release.',
      'Release Triage'   => 'Seat-level table of status, inbox depth, and current action — the fastest way to find which agent is blocked or waiting for input.',
      'System health first' => 'The recommended sequence: confirm ticks are healthy (Session page) and parity is OK (Parity page) before making workflow decisions here.',
    ]);
  ...
  public function langGraphFeatureProgress(): array {
    ...
    $build['context_banner'] = $this->renderLangGraphContextBanner(
      'Feature/work-item ownership and status across all org workstreams — every feature tracked in copilot-hq, with assigned PM/Dev/QA seats, current status, and priority. Paired with a live telemetry/parity summary so you know whether planning data can be trusted.',
      'Use during sprint planning, prioritisation reviews, or when a stakeholder asks what is in-flight. If parity or tick data shows ATTENTION, treat feature status as advisory only until the engine recovers.',
      'dashboards/FEATURE_PROGRESS.md — auto-generated by the orchestrator each tick from all features/*/feature.md status fields across all products.'
    );
    $build['terms'] = $this->renderLangGraphKeyTerms([
      'Work item'           => 'A trackable feature or task defined in a feature.md file under copilot-hq/features/. Each has a unique slug, a site, a module, and assigned agent seats.',
      'Status: ready'       => 'Feature is fully groomed (acceptance criteria written, scope agreed) and waiting for development to start.',
      'Status: in_progress' => 'Development, QA, or PM work is actively happening on this feature right now.',
      'Status: done'        => 'Code complete and merged. Not yet in a shipped release.',
      'Status: deferred'    => 'Feature was postponed — not currently being worked and not in a planned release.',
      'Status: shipped'     => 'Feature is deployed to production in a completed release.',
      'Priority P0/P1/P2/P3' => 'P0 = critical/blocking (rare). P1 = high-value, next to be started. P2 = normal queue. P3 = low priority / backlog.',
      'PM / Dev / QA'       => 'The agent seat IDs responsible for product management, development, and quality assurance on this feature.',
    ]);
  ...
  public function langGraphParityHealth(): array {
    ...
    $build['context_banner'] = $this->renderLangGraphContextBanner(
      'Parity validation result from the last orchestrator tick — whether the running graph matches its expected configuration: same agents selected, same pipeline steps in the same order.',
      'Use before approving a release (confirm PASS), after modifying engine.py or agents.yaml (confirm no unexpected divergence), or when the dashboard shows a parity FAIL to diagnose the mismatch.',
      'langgraph-parity-latest.json — overwritten every tick by the orchestrator parity checker. Contains parity_ok, expected/actual step lists, agent selection match, and error details.'
    );
    $build['terms'] = $this->renderLangGraphKeyTerms([
      'parity_ok'               => 'Overall pass/fail. True = the engine is behaving as configured. False = at least one check failed — see errors and individual match fields below.',
      'selected_agents.match'   => 'Whether the set of agents the orchestrator selected this tick exactly matches the expected set from the parity snapshot.',
      'steps.match'             => 'Whether the pipeline steps that executed this tick exactly match the expected step list from the parity snapshot (same steps, same order).',
      'generated_at'            => 'When the parity snapshot was last written. If this is old, the parity file may be stale — check Session Health for tick freshness.',
      'Parity snapshot'         => 'A reference configuration recorded when the engine was known-good. The current tick is compared against this snapshot to detect drift.',
      'Parity failure causes'   => 'Common causes: agents.yaml changed (new/removed agents), engine.py changed (new/removed nodes or edges), or orchestrator ran in a degraded mode that skipped steps.',
    ]);
  ...
  public function langGraphReleaseStatus(): array {
    ...
    $build['context_banner'] = $this->renderLangGraphContextBanner(
      'Release readiness posture — whether publishing and release cycle automation are both enabled, how many ticks in the last 24 hours had publishing enabled, and current engine mode.',
      'Use before approving a coordinated push or release, or when publishing appears paused. The 24h tick continuity count tells you how consistently the publish pipeline has been running.',
      'langgraph-ticks.jsonl (all ticks for 24h publish count + latest tick for current state) + release-cycle-control.json (manual enable/disable state).'
    );
    $build['terms'] = $this->renderLangGraphKeyTerms([
      'publish_enabled'          => 'When true in the latest tick, the orchestrator ran its publish step and wrote agent tracker telemetry to Drupal. When false, UI data is stale.',
      'release_cycle_enabled'    => 'Whether the release_cycle + coordinated_push pipeline steps are running. Disable this to pause release automation without stopping agent dispatch.',
      'dry_run'                  => 'When true, no external writes occurred — the orchestrator ran in safe mode. A dry-run tick does NOT count as a publishing tick even if publish_enabled=true.',
      'agent_cap'                => 'Max concurrent agent dispatches this tick. Relevant to release velocity — higher cap = faster release throughput.',
      'Ticks with publish_enabled (24h)' => 'Count of ticks in the last 24 hours where publish_enabled=true. A low count means the Drupal UI has been showing stale data for most of that window.',
    ]);
  ...
  public function langGraphReleaseNotes(): array {
    ...
    $build['context_banner'] = $this->renderLangGraphContextBanner(
      'Release notes and signoff artifacts for the current in-flight or most recently shipped release — the evidence package operators review before approving or communicating a release.',
      'Use after Release Control shows publish_enabled and release_cycle_enabled are both on, and before granting final Board approval. Confirms the narrative, change list, and QA/PM signoffs are complete.',
      'Artifact files written by PM and QA agents to copilot-hq sessions/*/artifacts/release-signoffs/. Also pulls from release outbox summaries and the releaseNotes() helper which reads RELEASE_STATE.md / orchestrator state.'
    );
    $build['terms'] = $this->renderLangGraphKeyTerms([
      'Release Evidence'   => 'The combined package of release notes, PM signoff, and QA signoff artifacts that must exist before a release can be considered complete.',
      'release_id'         => 'Unique identifier for a release cycle — typically a date-scoped slug like "20260408-forseti-release-d". Appears in signoff artifact filenames.',
      'PM signoff'         => 'Artifact written by the PM agent confirming scope is complete and feature list is accurate.',
      'QA signoff'         => 'Artifact written by the QA agent confirming test suites passed and no known blocking defects remain.',
      'Release narrative'  => 'Human-readable summary of what changed, who it affects, and why — authored by the PM agent as part of the release outbox.',
      'push-ready'         => 'Outbox item status meaning the PM agent has confirmed scope is complete and is requesting a GitHub push. The CEO/orchestrator gates this.',
    ]);
  ...
  public function langGraphReleaseTroubleshooting(): array {
    ...
    $build['context_banner'] = $this->renderLangGraphContextBanner(
      'Seat-level workload and state table — every registered agent seat with its current status, active work item, and inbox depth. The fastest way to find where a release is stalled.',
      'Use when a release is not advancing and you need to find which seat is blocked or waiting for input. Click through to an individual agent page to see their full inbox.',
      'copilot_agent_tracker_agents database table — populated by the orchestrator publish step each tick from session outbox and metadata files.'
    );
    $build['terms'] = $this->renderLangGraphKeyTerms([
      'Seat / Agent ID'    => 'A named orchestrator role — e.g. dev-forseti, pm-dungeoncrawler, qa-infra. Each seat has an inbox (queued work) and an outbox (completed work).',
      'Status: blocked'    => 'The agent has an inbox item it cannot complete — typically waiting on a Board decision, a dependency, or a prerequisite not yet done.',
      'Status: needs-info' => 'The agent sent a clarification request to the Board (you) and is waiting for a response before it can proceed.',
      'Status: working'    => 'The agent is actively running a Copilot CLI session on its current inbox item.',
      'Status: idle'       => 'The agent has no active inbox item. If inbox depth > 0 and status is idle, the orchestrator may not be dispatching to this seat.',
      'Inbox depth'        => 'Number of pending work items queued for this seat. High depth with "idle" status may indicate the orchestrator is not dispatching.',
      'current_action'     => 'The last action the agent reported — what it is currently doing or last did before becoming idle.',
      'Remediated'         => 'Seats the health_check pipeline step automatically nudged this tick (e.g. re-dispatched a stalled idle agent).',
    ]);

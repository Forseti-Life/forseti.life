 ...
 *
 * @QueueWorker(
 *   id = "job_application_automation_profile_text_extraction",
...
class ProfileTextExtractionWorker extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data) {
    ...
    if (!$profile || !$user || !$file) {
      \Drupal::logger('job_application_automation')->error('Failed to load profile @pid, user @uid or file @fid for text extraction', [
        '@pid' => $profile_id,
        '@uid' => $user_id,
        '@fid' => $file_id
      ]);
  ...
  private function extractResumeTextToProfile(Profile $profile, File $file) {
    if (!$profile->hasField('field_primary_resume_text')) {
      \Drupal::logger('job_application_automation')->error('Profile @pid does not have field_primary_resume_text field', [
        '@pid' => $profile->id()
      ]);
      return;
    }
    ...
    try {
      \Drupal::logger('job_application_automation')->info('Starting profile text extraction for file: @filename (Type: @mime_type)', [
      ...
      switch ($mime_type) {
        ...
        default:
          \Drupal::logger('job_application_automation')->warning('Unsupported file type for text extraction: @mime_type', [
            '@mime_type' => $mime_type
          ]);
          return;
      }
      ...
      if (!empty($extracted_text)) {
        ...
        \Drupal::logger('job_application_automation')->info('Successfully extracted @chars characters of text to profile @pid', [
      ...
      \Drupal::logger('job_application_automation')->error('Error extracting text from resume file: @message', [

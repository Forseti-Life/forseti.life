...
Notes:
- Local/dev `BASE_URL` (use this for continuous audits): `http://localhost` (or your local vhost URL if different).
- Production `BASE_URL` (reference only — do NOT run recursive crawls against prod unless explicitly authorized): `https://forseti.life`.
- The operator runner `scripts/site-audit-run.sh` enforces this: it refuses `https://forseti.life` unless `ALLOW_PROD_QA=1` is explicitly set.
- Preferred execution (local/dev): `FORSETI_BASE_URL=http://localhost ./scripts/site-audit-run.sh forseti-life`
...
```bash
# From: /home/keithaumiller/copilot-sessions-hq (HQ root)
...
$DRUSH --uri=http://localhost jhtr:qa-users-ensure --roles=authenticated
QA_UID=$($DRUSH --uri=http://localhost user:information qa_tester_authenticated --format=json \
  | python3 -c "import json,sys; d=json.load(sys.stdin); print(list(d.keys())[0])")
ULI=$($DRUSH --uri=http://localhost user:login --uid=$QA_UID --no-browser)

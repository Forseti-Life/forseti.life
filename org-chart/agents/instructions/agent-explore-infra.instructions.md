...
1. Check if your most recent outbox file is a stub (≤5 lines or contains "Missing required status header"). If so, write a remediation outbox before processing inbox items.
2. Run `bash scripts/sla-report.sh` — note any BREACHes for `agent-explore-infra`; if present, your current cycle's outbox (with `- Status: done`) will clear them.
3. Run `bash scripts/lint-scripts.sh` and `python3 scripts/qa-suite-validate.py` as baseline operator audit.

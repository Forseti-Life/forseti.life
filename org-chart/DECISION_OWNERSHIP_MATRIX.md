...
| Cross-module dependency or ownership boundary conflict | Owning PM + Owning Dev | CEO (if unresolved) | Requesting PM, QA | Affected teams | No (if unresolved in one handoff) | No agreement after one passthrough cycle |
| Security/privacy finding (authz, data exposure, secrets) | Dev | Security Analyst + PM | QA, CEO | Human owner (as needed) | Limited | Severity high/critical, unclear mitigation, or compliance impact |
| Release gate failure (QA BLOCK at Gate 2/4) | Dev (fix), QA (verify) | PM | CEO | Human owner (if persistent) | Yes (single-cycle fix path) | 3 unclean cycles, repeated identical failures, or no viable rollback |
| Production outage / severe regression | Dev | CEO | PM, QA, Security | Human owner | No | Immediate CEO/human escalation after containment starts |
| Missing access/credentials/environment path | Agent owning task | CEO | PM (if scope impact) | Affected team | No | Blocker persists >1 execution cycle |
| Process/rule ambiguity (who decides?) | Current seat | CEO | Relevant role owner | Team | No | Any ambiguity that blocks execution >1 cycle |
| Coordinated release go/no-go (multi-site) | PMs + Dev + QA | Release operator PM (`pm-forseti`) | CEO | Human owner | No | Any required signoff missing or rollback plan incomplete |

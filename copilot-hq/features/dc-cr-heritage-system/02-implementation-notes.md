...
- Targeted scenarios:
  - Submit step 1 without heritage → validation error inline
  - Submit with dwarf ancestry + elf heritage id → "Invalid heritage for selected ancestry."
  - Submit with dwarf ancestry + `forge` heritage → success
  - Human ancestry → only `versatile-heritage` is valid
  - Anonymous access to /characters/create → 403

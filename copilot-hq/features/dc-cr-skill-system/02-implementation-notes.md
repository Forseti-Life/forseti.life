...
## Summary
EXTEND: `CharacterCalculator` has `calculateProficiencyBonus()` with a TODO body; `CharacterStateService` stores character JSON including skills. Gaps: no SKILLS constant (17 skills with ability associations), no `calculateSkillCheck()` method, no `GET /character/{id}/skills` endpoint, and Lore skill specialization is not explicitly supported. First slice adds the SKILLS constant and implements `calculateSkillCheck()` in `CharacterCalculator`.

...
# Runs PM+CEO process review dispatch once per day (best-effort) and logs.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}" )/.." && pwd)"
cd "$ROOT_DIR"

PIDFILE=".improvement-round-loop.pid"
LOGDIR="inbox/responses"
LATEST="$LOGDIR/improvement-round-latest.log"
mkdir -p "$LOGDIR"
...
case "$cmd" in
  ...
  run)
    ...
    while true; do
      ...
      ts="$(date -Iseconds)"
      day="$(date +%Y%m%d)"
      daylog="$LOGDIR/improvement-round-${day}.log"

      out="$(./scripts/improvement-round.sh "$day" 2>&1 || true)"
      status="$(./scripts/improvement-round-status.sh "$day" 2>&1 || true)"

...
PIDFILE=".publish-forseti-agent-tracker-loop.pid"
LOGDIR="inbox/responses"
LATEST="$LOGDIR/publish-forseti-agent-tracker-latest.log"
mkdir -p "$LOGDIR"
...
case "$cmd" in
  ...
  run)
    ...
    while true; do
      ...
      ts="$(date -Iseconds)"
      daylog="$LOGDIR/publish-forseti-agent-tracker-$(date +%Y%m%d).log"
      out=$(./scripts/publish-forseti-agent-tracker.sh 2>&1 || true)

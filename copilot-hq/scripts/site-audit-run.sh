...
# Safety: QA automation must target local/dev environments by default.
# Override by setting FORSETI_BASE_URL / DUNGEONCRAWLER_BASE_URL.
# If you explicitly need to audit production, set ALLOW_PROD_QA=1.
FORSETI_BASE_URL="${FORSETI_BASE_URL:-http://localhost}"
DUNGEONCRAWLER_BASE_URL="${DUNGEONCRAWLER_BASE_URL:-http://localhost:8080}"
  ...
  case "$base_url" in
    https://forseti.life|https://dungeoncrawler.forseti.life)
      if [ "$ALLOW_PROD_QA" != "1" ]; then
        ...
        echo "Set FORSETI_BASE_URL/DUNGEONCRAWLER_BASE_URL to a local/dev URL (preferred)," >&2
        echo "or set ALLOW_PROD_QA=1 to explicitly allow production." >&2
        exit 2
      fi
      ;;
  esac
...
PY
  ...
  while IFS=$'\t' read -r team_id audit_filter base_url_env local_base_url production_url drupal_web_root qa_artifacts_dir route_regex qa_permissions_path dev_agent_id pm_agent_id; do
    ...
    # Priority:
    #   ALLOW_PROD_QA=1: production_url (product registry) > base_url_env env override > http://localhost
    #   normal:          local_base_url (product registry) > http://localhost
    ...
    if [ "$ALLOW_PROD_QA" = "1" ]; then
      if [ -n "$production_url" ]; then
        ...
        base_url="${env_value:-http://localhost}"
      else

...
REVIEW_DIR="knowledgebase/reviews/cycle/${DATE_YYYYMMDD}"
mkdir -p "$REVIEW_DIR/feedback"

# CEO summary
cp -n templates/daily-review.md "$REVIEW_DIR/cycle-review.md"
sed -i "s/YYYY-MM-DD/${DATE_ISO}/g" "$REVIEW_DIR/cycle-review.md"
...
for a in "${AGENTS[@]}"; do
  ...
  mkdir -p "sessions/${a}/inbox/${DATE_YYYYMMDD}-cycle-review"
  cat > "sessions/${a}/inbox/${DATE_YYYYMMDD}-cycle-review/README.md" <<EOM
# Release-Cycle Review Request — ${DATE_ISO}

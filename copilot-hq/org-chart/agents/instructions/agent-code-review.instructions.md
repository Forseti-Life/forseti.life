...
- [ ] Drupal-specific CSRF token delivery channel: for `_csrf_token: 'TRUE'` routes, the token MUST be delivered as a query param (`?token=`) in the form action URL — `RouteProcessorCsrf::processOutbound()` appends it automatically when using `Url::fromRoute(...)->toString()` or Twig `path()`. Hidden form fields (`name="token"` or `name="form_token"` in POST body) are dead code: `CsrfAccessCheck` reads `$request->query->get('token')` only. Flag any such hidden inputs as LOW dead code and recommend removal. Verify: `grep -n 'name.*form_token\|name.*"token"' templates/*.twig`. (2026-04-10: cover-letter-display, interview-prep-page, saved-searches-page in forseti-release-b all had dead hidden token fields)

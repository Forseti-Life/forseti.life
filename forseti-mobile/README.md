...
Google Maps Android key must **not** be hardcoded in tracked files.

Set it via one of these methods before Android builds:

1. Environment variable:

```bash
export FORSETI_MAPS_API_KEY="<your-google-maps-android-key>"
```

2. Gradle property in `android/gradle.properties` (local only, do not commit):

```properties
FORSETI_MAPS_API_KEY=<your-google-maps-android-key>
```

The Android manifest reads this through Gradle manifest placeholders.
...
**Project Repository**: https://github.com/Forseti-Life/forseti.life

/**
 * Basic Drupal Authentication Service for AmISafe Mobile
 * 
...
class DrupalAuthService {
  ...
  async initializeFromStorage() {
    try {
      const storedUser = await AsyncStorage.getItem('amisafe_user');
      const storedSession = await AsyncStorage.getItem('amisafe_session');
      const storedCsrf = await AsyncStorage.getItem('amisafe_csrf');
    ...
    try {
      ...
      if (response.status === 302 || response.status === 200) {
        ...
        await AsyncStorage.multiSet([
          ['amisafe_session', this.sessionToken || ''],
          ['amisafe_csrf', this.csrfToken || ''],
          ['amisafe_user', JSON.stringify(this.currentUser)]
        ]);
  ...
  async demoLogin(username, password) {
    try {
      ...
      if (username && password) {
        ...
        await AsyncStorage.setItem('amisafe_user', JSON.stringify(this.currentUser));
    ...
    try {
      ...
      if (username && password) {
        this.currentUser = {
          ...
          roles: ['authenticated', 'amisafe_user']
        };
